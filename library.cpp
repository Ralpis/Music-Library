...
void load(FILE *fp) {
	...
	while (1) {
		...
		if (strcmp(name, " ")==0) {
			...
			name = strdup(name); // buffer 가 지워지면 사라지기때문에 strdup로 복사
		}
...
void insert_to_index_directory(Song *ptr_song) {
	...
	//add the snode into the single linked list at index_talbe[index]
